<template>
  <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white w-full max-w-6xl rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden relative">
      ...
      <div class="absolute -top-24 -left-24 w-64 h-64 bg-primary/5 rounded-full blur-[80px] pointer-events-none"></div>
        ...
        <div>
          <div class="flex items-center gap-4 mb-2">
            ...
            <h2 class="text-2xl font-black text-slate-800 tracking-tight uppercase">
              Mobile App <span class="text-primary font-light">—</span> Daily Report
            </h2>
          </div>
          <p class="text-slate-400 text-[10px] font-black tracking-[0.3em] uppercase ml-16">Live Channel Performance Analytics</p>
        </div>
      ...
      <div v-if="loading" class="h-[520px] flex flex-col items-center justify-center bg-slate-50/50">
        ...
        <p class="text-slate-500 font-bold tracking-widest uppercase animate-pulse">Fetching Analytics...</p>
      </div>
      ...
      <div v-else-if="error" class="h-[520px] flex items-center justify-center">
        <div class="bg-red-50 border border-red-100 p-10 rounded-[2rem] text-center max-w-md">
          ...
          <button @click="getDailyAppReport" class="w-full bg-red-500 hover:bg-red-600 text-white py-3 rounded-xl font-black uppercase tracking-widest transition-all">Retry Connection</button>
        </div>
      </div>
      ...
      <div v-else class="relative px-10 pb-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-6">
          <div class="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 w-full md:w-auto">
            <button 
              @click="tab = 'counts'" 
              :class="tab === 'counts' ? 'bg-white text-primary shadow-md scale-[1.02]' : 'text-slate-500 hover:text-slate-700'"
            ...
            <button 
              @click="tab = 'amounts'" 
              :class="tab === 'amounts' ? 'bg-white text-primary shadow-md scale-[1.02]' : 'text-slate-500 hover:text-slate-700'"
              class="flex-1 md:flex-none px-10 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-300"
            >
              Amounts
            </button>
...
const colors = ["#F59E0B",  "#3B82F6", "#10B981", "#EF4444", "#8B5CF6", "#14B8A6", "#EC4899"];

const cleanName = (key) =>
  key.replace(/^m_/, "").replace(/_count|_sum$/, "").replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase());

const getDailyAppReport = async () => {
  try {
    loading.value = true;
    error.value = null;
    const res = await api.get("/cron_local_report/app/report", { type: "daily", date: props.date });
    raw.value = { hours: res.hours || [], series: res.series || {} };
  } catch (err) {
    error.value = err?.response?.data?.message || "Connection to analytics server failed";
  } finally {
    loading.value = false;
  }
};
...
const countSeries = computed(() =>
  Object.keys(raw.value.series)
    ...
    .map(k => ({ name: cleanName(k), data: sliceData(raw.value.series[k] || []) }))
);
...
/* Updated Chart Options for Light Background */
